...
import { resultsupabase } from './resultsupabaseClient';
// For Results (Old DB)
import { questionSupabase } from './questionSupabaseClient';
// For Questions (New DB)
import { 
  Play, 
  Clock, 
  CheckCircle, 
...
export default function App() {
  ...
  // --- EFFECT: LOAD QUESTIONS FROM SUPABASE ---
  useEffect(() => {
  ...
  const handleStartProcess = () => {
    ...
    // Immediately trigger test workflow bypassing database token authentication checks
    ...
    return (
      <div className="fixed inset-0 bg-gray-50 flex items-center justify-center p-4 font-sans text-gray-900 overflow-hidden">
        <div className="bg-white p-5 md:p-8 rounded-2xl shadow-xl max-w-xl w-full text-center border border-blue-100 relative flex flex-col justify-center shrink-0 max-h-full overflow-y-auto">
          ...
          {/* Test Description (Time limit, questions and scoring) */}
          <div className="text-left bg-gray-50 p-3 rounded-lg text-xs text-gray-700 mb-4 space-y-1">
            <p className="flex items-center">
              <Clock className="w-3.5 h-3.5 mr-2"/> 
              <strong>Time Limit:</strong> {selectedTopic ? `${testDuration} Minutes` : "Select a topic first"}
            </p>
            <p className="flex items-center">
              <FileText className="w-3.5 h-3.5 mr-2"/> 
              <strong>Questions:</strong> {selectedTopic ? (isLoadingQuestions ? 'Loading...' : questions.length) : "Select a topic first"}
            </p>
            <p className="flex items-center">
              <Tag className="w-3.5 h-3.5 mr-2"/> 
              <strong>Scoring:</strong> {selectedTopic ? `${marksPerQuestion} Marks / Question` : "Select a topic first"}
            </p>
          </div>
                                            ...
                                            <div className={`flex items-start p-2 rounded ${isCorrect ? 'bg-green-50 text-green-800' : 'bg-red-50 text-red-800'}`}>
                                                ...
                                                <span className="break-words">
                                                   {skipped ? <span className="italic text-gray-500">Skipped</span> : 
                                                        <span><span className="font-bold uppercase mr-1">{userAns.replace('option','')}</span><Latex>{q[userAns]}</Latex></span>
                                                    }
